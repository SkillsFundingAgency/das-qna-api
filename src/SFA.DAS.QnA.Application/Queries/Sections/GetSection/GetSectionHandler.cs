...
namespace SFA.DAS.QnA.Application.Queries.Sections.GetSection
{
    public class GetSectionHandler : IRequestHandler<GetSectionRequest, HandlerResponse<Section>>
    {
        ...
        {
            section.QnAData.Pages.RemoveAll(p => p.NotRequiredConditions != null && p.NotRequiredConditions.Any(nrc => nrc.IsOneOf != null && nrc.IsOneOf.Contains(applicationData[nrc.Field]?.Value<string>())));

            section.QnAData.Pages.RemoveAll(p => p.NotRequiredConditions != null && p.NotRequiredConditions.Any(nrc => nrc.IsNotOneOf != null && !nrc.IsNotOneOf.Contains(applicationData[nrc.Field]?.Value<string>())));
        }
    }
}